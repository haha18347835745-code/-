const app = {
    ...
    if (this.data.currentFile.isImage) {
      contentWrapper.innerHTML = `
        <div class="image-preview">
          <img class="preview-image" src="${file.fileID}" alt="preview">
        </div>
      `;
    } else {
      contentWrapper.innerHTML = `
        <div class="document-preview">
          ...
          <span class="doc-name">${file.title}</span>
        </div>
      `;
    }
  ...
  viewDocument() {
    alert('文档查看功能：在网页版中，文档将直接打开或下载');
    window.open(this.data.currentFile.fileID, '_blank');
  },
